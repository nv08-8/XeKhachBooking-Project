...
-- 1. Hàm xử lý cộng xu tự động
CREATE OR REPLACE FUNCTION reward_booking_coins()
RETURNS TRIGGER AS $$
BEGIN
    -- Chỉ cộng xu khi vé chuyển từ trạng thái khác sang confirmed hoặc completed
    -- OLD.status IS NULL hỗ trợ trường hợp insert mới với status có sẵn
    IF (NEW.status IN ('confirmed', 'completed')) AND (OLD.status IS NULL OR OLD.status != NEW.status) THEN
        -- Kiểm tra để tránh cộng xu 2 lần cho cùng 1 booking_id
...
-- 2. Gán trigger vào bảng bookings
